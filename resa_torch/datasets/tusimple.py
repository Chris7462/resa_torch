...
class TuSimple(Dataset):
    ...
    Args:
        root: Path to TuSimple dataset root directory
        image_set: One of 'train', 'val', or 'test'
        transforms: Optional albumentations transforms to apply to samples

    Note:
        Requires pre-generated segmentation labels. Run:
        python tools/generate_seg_tusimple.py --root /path/to/tusimple
    """
    ...
    def _load_annotations(self) -> None:
        ...
        list_file = os.path.join(self.root, 'seg_label', 'list', f'{self.image_set}_gt.txt')
        ...
        with open(list_file) as f:
            for line in f:
                ...
                self.exist_list.append([int(x) for x in parts[2:]])  # 6 lanes

    def _load_test_list(self) -> None:
        """Load image list for test set."""
        list_file = os.path.join(self.root, 'seg_label', 'list', 'test_gt.txt')
